...
const CreateListing = () => {
  ...
  const checkGeo = async () => {
    await axios
      .get(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${address}.json?access_token=pk.eyJ1IjoiamFldmllbiIsImEiOiJjbDFxZ3Y1b3UwOWVoM2ttejVrMWMxemc0In0.8Wg7OEONd7l1dkmei_uCRA`
      )
      .then((res) => {
        console.log(res.data.features[0].place_name);
        setLng(res.data.features[0].center[0] ?? 0);

        setLat(res.data.features[0].center[1] ?? 0);
        setGot(!got);
      });
  };
  const onSubmit = async (e) => {
    ...
    if (geoLocEnabled) {
      ...
      await axios
      .get(`https://api.mapbox.com/geocoding/v5/mapbox.places/${address}.json?access_token=pk.eyJ1IjoiamFldmllbiIsImEiOiJjbDFxZ3Y1b3UwOWVoM2ttejVrMWMxemc0In0.8Wg7OEONd7l1dkmei_uCRA`)
        // .get(   `http://api.positionstack.com/v1/forward?access_key=0d3509329d6e66668cba1ec6521edf80&query=${address}` )
        ...
        setLat(res.data.features[0].center[1] ?? 0);
          ...
          geolocation.lng = res.data.features[0].center[0] ?? 0;
          
          geolocation.lat = res.data.features[0].center[1] ?? 0;
